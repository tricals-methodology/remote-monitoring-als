# Estimate non-wear time based on Choi et al (2011) paper

## Description 
## 1-min time intervals with consecutive zero counts for at least 90-min time window (window 1), allowing a short time intervals with nonzero counts lasting up to 2 min (allowance interval) if no counts are detected during both the 30 min (window 2) of upstream and downstream from that interval; any nonzero counts except the allowed short interval are considered as wearing

## Hyperparameters based on Syed et al (2020) paper; code adapted from https://github.com/shaheen-syed/ActiGraph-ActiWave-Analysis
...
choi_2011_calculate_non_wear_time <- function (data, 
  ...
  #Any count that is above the activity threshold is considered a spike. The tolerence defines the number of spikes that are acceptable within a sequence of zeros. The default is 2, meaning that we allow for 2 spikes in the data, i.e. artificial movement
  ...
  #if set to True, then print the output of the non wear sequence, start index, end index, duration, start time and end time Default is False
      ...
      if (nval > 0) {
        ...
        if (n == nrow(aggr)){
          ...
          upstream <- aggr[(n + spike_tolerance):ifelse (
            ...
            n + min_window_len + (spike_tolerance - 1)), "VM"]
        }
  ...
  if (nrow (ranges) > 0){
    ...
    for (r in 1:nrow (ranges)) {
      ...
      if (print_output == TRUE) {
        ...
        print(paste("No. no wear:", r, "start aggregated index:", ranges[r, 1], "end aggregated index:", 
                    ranges[r, 2], "length:", ranges[r, 2] - ranges[r, 1]))
        print(paste("start time:", aggr[ranges[r, 1], "DATE"], "end time:", aggr[ranges[r, 2], "DATE"], 
